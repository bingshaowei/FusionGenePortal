...
from routes.fusions import (
    fusion_bp,
    _load_suggestion_cache,
    _load_top_fusions_cache,
    _get_co_occurrence_sample_index,
)
from routes.transcriptome import (
    transcriptome_bp,
...
with app.app_context():
    ...
    # 2b. Co-occurrence sample index. This is intentionally preloaded so the
    # fusion detail page does not pay the sample-index build cost on first view.
    try:
        print(">>> [2b/7] Preloading co-occurrence sample index...")
        co_index = _get_co_occurrence_sample_index()
        print(f">>> ✅ Co-occurrence sample index preload complete: {len(co_index)} samples")
    except Exception as e:
        print(f">>> ⚠️  Co-occurrence sample index preload failed: {e}")
    ...
    # 6. 转录组 TPM 矩阵预加载
    try:
        print(">>> [6/7] 预加载转录组 TPM 矩阵 normalized.all.count.csv...")
        expr_df = load_expression_matrix()
        if expr_df is not None:
            print(f">>> ✅ TPM 矩阵预加载完成: {expr_df.shape[0]} 基因 × {expr_df.shape[1]} 样本")
        else:
            print(">>> ⚠️  TPM 矩阵未找到或加载失败")
    except Exception as e:
        print(f">>> ⚠️  TPM 矩阵预加载失败: {e}")

    # 7. 转录组 raw count 矩阵预加载
    try:
        print(">>> [7/7] 预加载转录组 raw count 矩阵 raw_count.csv...")
        count_df = load_count_matrix()
        if count_df is not None:
            print(f">>> ✅ raw count 矩阵预加载完成: {count_df.shape[0]} 基因 × {count_df.shape[1]} 样本")
        else:
            print(">>> ⚠️  raw count 矩阵未找到或加载失败")
    except Exception as e:
        print(f">>> ⚠️  raw count 矩阵预加载失败: {e}")

    print(">>> ✅ GDSC drug sensitivity data has already been preloaded in the master process")
